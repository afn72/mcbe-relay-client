...
const realmname = 'survival realm'
...
const main = async () => {
  const relay = new Relay({
    ...
    destination: {
      realms: {
        // pickRealm: (realms) => realms.find(realm => realm.name.toLowerCase().includes(realmname)),
        realmInvite: code
      }
    }
  });
  ...
  relay.on('connect', (player) => {
    console.log(`Connected from ${player.connection.address}`);
        ...
        const fp = (p) => JSON.stringify(p, (k, v) => typeof v === 'bigint' ? v.toString() : v);
        
        player.on('clientbound', ({ name, params }, des) => {
          /* if (fp(params).toLowerCase().includes('particle')){
            console.log(name, fp(params))
          } */
          if (name === 'move_player'){
